...
func TestRunToolSubagentModelHierarchyAndUsageAttributes(t *testing.T) {
	...
	parentJournal.Observe(runtime.Entry{Bucket: runtime.BucketMiddleware, CallID: "middleware", Usage: model.Usage{InputTokens: 4, OutputTokens: 1}})
	...
	parentState := middleware.NewState(middleware.StateInit{RunID: "run-1", ThreadID: "thread-1", AssistantID: "lead"})
	...
	definition := m.InstrumentToolDefinition(tool.Definition{
		...
		Handler: func(handlerCtx context.Context, _ tool.Call) (*tool.Result, error) {
			...
			childState := middleware.NewState(middleware.StateInit{
	...
	for key, want := range map[string]int64{
		"usage.lead_tokens": 3, "usage.subagent_tokens": 2, "usage.middleware_tokens": 5,
	} {
...
func TestSubagentUsesExplicitParentRunIDWithoutPropagatedSpanContext(t *testing.T) {
	...
	parentState := middleware.NewState(middleware.StateInit{RunID: "parent", ThreadID: "thread"})
	...
	childState := middleware.NewState(middleware.StateInit{RunID: "parent:task-1", ThreadID: "thread"})
