// Package middleware 定义四阶段中间件契约、链的执行语义与声明式锚点排序。
//
// 编排不在这里：回合的步骤顺序（压缩 → 裁剪 → 工具集 → 采样 → 落存 → 工具 → 停止判定）
// 归 pkg/loop 的 for 循环。中间件只承载横切关注点（设计文档 §3、§4）。
package middleware
...
// Directive 是中间件对内核控制流的请求。
//
// 内核读它，中间件写它。这样澄清拦截、Schema 校验重试这类需求不必让内核特判
// 具体中间件（设计文档 §4.2）。
type Directive int
...
// State 是中间件与内核之间的共享状态。
//
// 内置链需要的一切都是具体字段，不是 map 键。Values 保留给树外第三方中间件，
// 树内代码一次都不用 —— 一旦所有步骤都通过 map[string]any 通信，
// 就会出现时序耦合（中间件 B 只在 A 先跑过时才工作），而 Go 连动态类型那点
