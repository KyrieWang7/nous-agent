...
//
// GradeAbort 只覆盖"中间件本身返回 error"，而它不会：单个 hook 的执行失败
// 由 Runner 吞掉并告警（hook 是治理增强，不是授权源）。
func (h *Hook) Grade() middleware.Grade { return middleware.GradeAbort }

// BeforeTool 实现 middleware.BeforeTool。
func (h *Hook) BeforeTool(ctx context.Context, st *middleware.State) (tool.Decision, error) {
...
func (h *Hook) AfterTool(ctx context.Context, st *middleware.State) error {
