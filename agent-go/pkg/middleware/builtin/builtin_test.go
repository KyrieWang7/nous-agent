...
func state() *middleware.State {
	return middleware.NewState(middleware.StateInit{History: message.NewHistory(), RunID: "run-1"})
}

func withCall(name, args string) *middleware.State {
	st := state()
...
func TestSandboxAudit_BlocksDangerousCommands(t *testing.T) {
	...
	audit := builtin.NewSandboxAudit(builtin.AuditOptions{})
...
func TestSandboxAudit_IgnoresNonCommandFields(t *testing.T) {
	...
	audit := builtin.NewSandboxAudit(builtin.AuditOptions{Tools: []string{"bash"}})
...
func TestSandboxAudit_GradeIsAbort(t *testing.T) {
	...
	if got := builtin.NewSandboxAudit(builtin.AuditOptions{}).Grade(); got != middleware.GradeAbort {
...
func TestToolErrorHandling_ConvertsExecErrorToErrorResult(t *testing.T) {
	...
	if err := builtin.NewToolErrorHandling().AfterTool(context.Background(), st); err != nil {
...
// 已被标记为错误的结果不重复包装：上游中间件可能已经加工过，覆盖会丢掉它们的工作。
...
func TestToolOutputBudget_LeavesSmallOutputAlone(t *testing.T) {
	...
	b := builtin.NewToolOutputBudget(builtin.BudgetOptions{ExternalizeMinChars: 100})
...
func TestToolOutputBudget_ExternalisesLargeOutput(t *testing.T) {
	...
	b := builtin.NewToolOutputBudget(builtin.BudgetOptions{
...
func TestToolOutputBudget_GradeIsListener(t *testing.T) {
	...
	b := builtin.NewToolOutputBudget(builtin.BudgetOptions{})
	if got := b.Grade(); got != middleware.GradeListener {
...
func TestPermissionMiddleware_DeniesWithReason(t *testing.T) {
	t.Parallel()
	...
	mw := builtin.NewPermission(policy, r)
	d, err := mw.BeforeTool(context.Background(), withCall("write_file", `{}`))
	if err != nil {
		t.Fatalf("BeforeTool() error = %v", err)
	}
	if !d.Deny {
...
func TestPermissionMiddleware_AllowsPermittedTool(t *testing.T) {
	t.Parallel()
	...
	policy, _ := permission.NewPolicy(permission.Config{Mode: permission.ModeReadOnly})
	mw := builtin.NewPermission(policy, r)

	d, err := mw.BeforeTool(context.Background(), withCall("read_file", `{}`))
	if err != nil {
		t.Fatal(err)
	}
	if d.Deny {
...
// 未注册的工具走到这里必须拒绝而不是放行。
func TestPermissionMiddleware_UnknownToolIsDenied(t *testing.T) {
	t.Parallel()

	policy, _ := permission.NewPolicy(permission.Config{Mode: permission.ModeAllow})
	mw := builtin.NewPermission(policy, tool.NewRegistry())

	d, err := mw.BeforeTool(context.Background(), withCall("ghost", `{}`))
	if err != nil {
		t.Fatal(err)
	}
	if !d.Deny {
