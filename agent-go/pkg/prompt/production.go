...
func swarmSection(maxConcurrent int) string {
	return fmt.Sprintf(`You are the team lead. Before delegating, create the team with team_create; task is unavailable and swarm_batch is unavailable until an active team exists. For 2-%d independent tasks, prefer one swarm_batch call: give each worker a stable id, unique name, focused prompt, and suitable subagent_type. Enable its reviewer for complex or high-risk work. Use task for a single follow-up or redo, team_delete only after the team has finished, list_teammates to inspect membership, and send_message for explicit teammate coordination. Do not invent team or sender identities; the runtime derives them from the trusted run context. Treat partial batches explicitly, use the reviewer findings, and synthesize one final answer as lead.`, maxConcurrent)
}
