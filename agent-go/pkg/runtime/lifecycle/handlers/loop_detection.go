...
// NameLoopDetection 是循环检测生命周期处理器的名字。
const NameLoopDetection = "loopDetection"

// LoopDetection 检测重复的工具调用并打断。
//
// 它是启发式保护，不是安全上限：内核里的 MaxIterations 才是不可删的兜底
// （设计文档 §3.3）。删掉本生命周期处理器只是少一层"提前发现模型卡住了"的能力。
//
// 判定是**无状态**的，每次从转录推导。生命周期处理器实例在服务端被多个会话共享，
...
// NewLoopDetection 返回循环检测生命周期处理器。
...
func (l *LoopDetection) Grade() lifecycle.Grade { return lifecycle.GradeListener }

// AfterModel 实现 lifecycle.AfterModel。
func (l *LoopDetection) AfterModel(_ context.Context, st *lifecycle.State) error {
