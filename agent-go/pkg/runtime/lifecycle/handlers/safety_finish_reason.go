...
// NameSafetyFinishReason 是安全终止处理生命周期处理器的名字。
...
// 于是本生命周期处理器先观察原始输出、清掉不安全的调用，LoopDetection 再对
// 清理后的消息计数（设计文档 §4.4）。
type SafetyFinishReason struct {
	reasons []string
}

// NewSafetyFinishReason 返回安全终止处理生命周期处理器。
//
...
func (SafetyFinishReason) Grade() lifecycle.Grade { return lifecycle.GradeAbort }

// AfterModel 实现 lifecycle.AfterModel。
func (s *SafetyFinishReason) AfterModel(_ context.Context, st *lifecycle.State) error {
