...
func TestDanglingToolCall_FillsMissingResults(t *testing.T) {
	...
	if err := handlers.NewDanglingToolCall().BeforeModel(context.Background(), st); err != nil {
...
func TestSafetyFinishReason_DropsToolCallsOnLength(t *testing.T) {
	...
	if err := handlers.NewSafetyFinishReason().AfterModel(context.Background(), st); err != nil {
...
func TestSafetyFinishReason_LeavesNormalResponsesAlone(t *testing.T) {
	...
	call := assistantCalls("a")
	st := lifecycle.NewState(lifecycle.StateInit{History: hist(user("q"), call)})
...
func TestSafetyFinishReason_CustomReasons(t *testing.T) {
	...
	if err := handlers.NewSafetyFinishReason("refusal").AfterModel(context.Background(), st); err != nil {
		t.Fatal(err)
	}
	if len(st.ModelOutput.Message.ToolCalls) != 0 {
...
func TestLoopDetection_BreaksRepeatedIdenticalCall(t *testing.T) {
	...
	ld := handlers.NewLoopDetection(handlers.LoopDetectionOptions{Threshold: 3, Window: 6})
...
func TestLoopDetection_DifferentArgumentsAreNotALoop(t *testing.T) {
	...
	st := lifecycle.NewState(lifecycle.StateInit{History: hist(msgs...)})
	st.ModelOutput = &model.Response{Message: msgs[len(msgs)-2], StopReason: model.StopReasonToolCalls}

	ld := handlers.NewLoopDetection(handlers.LoopDetectionOptions{Threshold: 3, Window: 10})
	if err := ld.AfterModel(context.Background(), st); err != nil {
		t.Fatal(err)
	}
	if len(st.ModelOutput.Message.ToolCalls) == 0 {
...
func TestLoopDetection_IsStatelessAcrossHistories(t *testing.T) {
	...
	st1 := lifecycle.NewState(lifecycle.StateInit{History: hist(loopy...)})
	...
	st2 := lifecycle.NewState(lifecycle.StateInit{History: hist(clean...)})
...
func TestLoopDetection_WindowLimitsLookback(t *testing.T) {
	...
	ld := handlers.NewLoopDetection(handlers.LoopDetectionOptions{Threshold: 3, Window: 4})
	if err := ld.AfterModel(context.Background(), st); err != nil {
		t.Fatal(err)
	}
	if len(st.ModelOutput.Message.ToolCalls) == 0 {
...
func TestLoopDetection_GradeIsListener(t *testing.T) {
	...
	ld := handlers.NewLoopDetection(handlers.LoopDetectionOptions{})
	if got := ld.Grade(); got != lifecycle.GradeListener {
