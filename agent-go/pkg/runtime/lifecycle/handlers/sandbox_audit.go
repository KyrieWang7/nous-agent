...
// NameSandboxAudit 是命令审计生命周期处理器的名字。
...
// Grade 实现 lifecycle.Graded：审计失败必须中断。
func (s *SandboxAudit) Grade() lifecycle.Grade { return lifecycle.GradeAbort }

// BeforeTool 实现 lifecycle.BeforeTool。
func (s *SandboxAudit) BeforeTool(_ context.Context, st *lifecycle.State) (tool.Decision, error) {
	if st.ToolCall == nil {
