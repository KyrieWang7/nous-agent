...
// TerminalName 是终端生命周期处理器的名字。
//
// 澄清拦截必须看到最终输出，所以它固定排在链尾。未声明锚点的扩展生命周期处理器
// 插到它之前（设计文档 §4.3）。
...
// Handler 是所有生命周期处理器的基础契约。
//
// Name 是生命周期处理器的身份：锚点排序、错误信息与"声明的生命周期处理器是否在链里"的校验
// 都靠它，所以链内不允许重名。
type Handler interface {
	Name() string
}

// 以下均为可选接口。生命周期处理器只实现自己需要的阶段，Dispatcher 靠类型断言收集。
...
//
// State.ToolExecErr 承载 Handler 的框架层错误，ToolErrorHandling 生命周期处理器
// 靠它把执行期失败转成 error ToolMessage。
type AfterTool interface {
	AfterTool(ctx context.Context, st *State) error
}

// Grade 是生命周期处理器失败时的处理级别。
type Grade int

const (
	// GradeAbort 表示该生命周期处理器失败必须中断回合。默认级别。
	//
	// 权限、护栏、Schema 校验、历史修复类生命周期处理器都属于此类：
	// 它们失败意味着"无法确认安全"或"请求会非法"，继续跑下去更糟。
	GradeAbort Grade = iota

	// GradeListener 表示该生命周期处理器只是旁路监听，失败只记日志。
	//
...
// Graded 让生命周期处理器声明自己的失败级别。未实现该接口的按 GradeAbort 处理 —— 默认保守。
type Graded interface {
	Grade() Grade
}

// Anchor 声明扩展生命周期处理器在链中的相对位置。After 与 Before 互斥。
type Anchor struct {
	After  string
	Before string
}

// Anchored 让扩展生命周期处理器声明锚点。未实现该接口的插到终端生命周期处理器之前。
type Anchored interface {
	Anchor() Anchor
}

func gradeOf(m Handler) Grade {
	if g, ok := m.(Graded); ok {
		return g.Grade()
	}
	return GradeAbort
