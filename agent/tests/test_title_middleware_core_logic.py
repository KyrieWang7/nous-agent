...
class TestTitleMiddlewareCoreLogic:
    ...
    def test_generate_title_fallback_extracts_text_from_content_blocks(self, monkeypatch):
        ...
        state = {
            "messages": [
                HumanMessage(content=[{"type": "text", "text": "你可以做什么"}]),
                AIMessage(content="你好，我是 Nous"),
            ]
        }
        title = middleware._generate_title(state)
