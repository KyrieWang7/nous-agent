...
class AioSandbox(Sandbox):
    ...
    def read_file_bytes(self, path: str) -> bytes | None:
        """Read the binary content of a file using the sandbox download API.

        Falls back to base64 shell command if download API returns empty data.
        """
        # Primary: try the download_file API
        try:
            chunks = self._client.file.download_file(path=path)
            data = b"".join(chunks)
            if data:
                return data
        except Exception as e:
            logger.warning(f"download_file API failed for {path}: {e}, trying base64 fallback")

        # Fallback: read via base64 shell command (works reliably for binary files)
        try:
            result = self._client.shell.exec_command(command=f"base64 -w 0 '{path}' 2>/dev/null")
            output = result.data.output if result.data else ""
            if output and not output.startswith("Error"):
                return base64.b64decode(output.strip())
            logger.warning(f"base64 fallback returned empty/error for {path}: {output!r}")
        except Exception as e:
            logger.error(f"Failed to read binary file via base64 fallback: {e}")
