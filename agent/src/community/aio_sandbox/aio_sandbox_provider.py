...
from .static_backend import StaticSandboxBackend
...
DEFAULT_PORT = 8080
DEFAULT_CONTAINER_PREFIX = "nous-ai-sandbox"
...
class AioSandboxProvider(SandboxProvider):
    ...
    def _create_backend(self) -> SandboxBackend:
        ...
        2. ``base_url`` set + ``auto_start`` false → StaticSandboxBackend
              Connects to a pre-existing sandbox (e.g., docker-compose service).
        ...
        if base_url and not self._config.get("auto_start", True):
            logger.info(f"Using static sandbox backend at {base_url}")
            return StaticSandboxBackend(base_url=base_url)
    ...
    def _get_extra_mounts(self, thread_id: str | None) -> list[tuple[str, str, bool]]:
        ...
        When running inside a Docker container (detected by /.dockerenv),
        thread data mounts are skipped (persisted via MinIO instead), but
        skills are still mounted since they are read-only static files.
        """
        in_container = self._is_running_in_container()
        mounts: list[tuple[str, str, bool]] = []

        if thread_id and not in_container:
            mounts.extend(self._get_thread_mounts(thread_id))
            logger.info(f"Adding thread mounts for thread {thread_id}: {mounts}")
        elif in_container:
            logger.info(
                "Running inside a container — skipping thread data mounts; "
                "files will be persisted via MinIO"
            )

        skills_mount = self._get_skills_mount(host_mode=in_container)
        if skills_mount:
    ...
    @staticmethod
    def _get_skills_mount(host_mode: bool = False) -> tuple[str, str, bool] | None:
        """Get the skills directory mount configuration.

        Args:
            host_mode: When True (running inside a container), resolve
                the host-side path for /app/skills via SKILLS_HOST_PATH
                env var or Docker self-inspection.
        """
        try:
            config = get_app_config()
            ...
            if host_mode:
                host_path = os.getenv("SKILLS_HOST_PATH")
                if not host_path:
                    host_path = AioSandboxProvider._detect_host_skills_path()
                if host_path:
                    logger.info(f"Using host skills path for sandbox mount: {host_path}")
                    return (host_path, container_path, True)
                logger.warning(
                    "Could not resolve host skills path — "
                    "skills will not be available in sandbox. "
                    "Set SKILLS_HOST_PATH env var to fix this."
                )
                return None

            skills_path = config.skills.get_skills_path()
            if skills_path.exists():
                return (str(skills_path), container_path, True)
        except Exception as e:
            logger.warning(f"Could not setup skills mount: {e}")
        return None

    @staticmethod
    def _detect_host_skills_path() -> str | None:
        """Auto-detect the host-side skills path by inspecting own container mounts."""
        try:
            import docker
            ...
            for mount in container.attrs.get("Mounts", []):
                if mount.get("Destination") == "/app/skills":
                    source = mount.get("Source", "")
                    if source:
                        logger.info(f"Auto-detected host skills path: {source}")
                        return source
        except Exception as e:
            logger.debug(f"Could not auto-detect host skills path: {e}")
        return None
