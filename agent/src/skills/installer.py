...
from src.skills.permissions import make_skill_tree_sandbox_readable
...
async def _scan_skill_file_or_raise(skill_dir: Path, path: Path, skill_name: str, *, executable: bool, fail_open: bool = False) -> None:
    ...
    try:
        ...
        if fail_open:
            logger.warning("Security scan errored for %s; allowing due to fail_open: %s", location, e)
            return
    ...
    # When the scanner is unavailable it returns a conservative "block". Under
    # fail_open we treat that specific degraded outcome as a warning so a model
    # outage does not break installs; genuine block decisions still apply when
    # the scanner actually ran (handled by the security_scanner reason text).
    if fail_open and decision == "block" and "unavailable" in reason.lower():
        logger.warning("Security scan unavailable for %s; allowing due to fail_open: %s", location, reason)
        return
...
async def _scan_skill_archive_contents_or_raise(skill_dir: Path, skill_name: str) -> None:
    """Run install-time security checks against a skill's installable files.

    The structural check (no nested SKILL.md) always runs. The LLM content scan
    only runs when ``skill_security.enabled`` is set; otherwise this is a no-op
    beyond the structural check, preserving the legacy install behaviour.
    """
    # Resolve security-scan config (gracefully default to disabled if unavailable).
    enabled = False
    fail_open = False
    try:
        from src.config import get_app_config

        sec = getattr(get_app_config(), "skill_security", None)
        if sec is not None:
            enabled = bool(sec.enabled)
            fail_open = bool(sec.fail_open)
    except Exception:
        logger.debug("Could not load skill_security config; defaulting to disabled", exc_info=True)

    skill_md = skill_dir / "SKILL.md"
    if enabled:
        await _scan_skill_file_or_raise(skill_dir, skill_md, skill_name, executable=False, fail_open=fail_open)
    ...
    for path in sorted(skill_dir.rglob("*")):
        ...
        if rel_path == Path("SKILL.md"):
            continue
        # Structural safety: a nested SKILL.md is never allowed, regardless of scanning.
        ...
        await _scan_skill_file_or_raise(skill_dir, path, skill_name, executable=_is_script_support_file(rel_path), fail_open=fail_open)
