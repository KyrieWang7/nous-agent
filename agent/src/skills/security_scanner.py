...
Ported from deer-flow (deerflow.skills.security_scanner). Adapted to nous-agent:
- uses ``src.models.create_chat_model`` (no ``app_config`` kwarg);
- reads the moderation model name from the optional ``skill_evolution`` config
  section if present, otherwise falls back to the default model.
"""
...
def _resolve_moderation_model_name(config) -> str | None:
    """Read the moderation model name from the optional skill_security config."""
    skill_security = getattr(config, "skill_security", None)
    if skill_security is not None:
        name = getattr(skill_security, "moderation_model_name", None)
        if name:
            return name
    # Back-compat: also honour a skill_evolution section if one exists.
    skill_evolution = getattr(config, "skill_evolution", None)
    if skill_evolution is not None:
        return getattr(skill_evolution, "moderation_model_name", None)
    return None
...
async def scan_skill_content(content: str, *, executable: bool = False, location: str = SKILL_MD_FILE) -> ScanResult:
    ...
    try:
        ...
        model_name = _resolve_moderation_model_name(config)
