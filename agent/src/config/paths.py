...
class Paths:
    ...
    def ensure_thread_dirs(self, thread_id: str) -> None:
        ...
        self.acp_workspace_dir(thread_id).mkdir(parents=True, exist_ok=True)

    def resolve_virtual_path(self, thread_id: str, virtual_path: str) -> Path:
        """Resolve a sandbox virtual path to the actual host filesystem path.

        Supports both ``/mnt/user-data/...`` and ``/mnt/acp-workspace/...``.

        Args:
            thread_id: The thread ID.
            virtual_path: Virtual path as seen inside the sandbox, e.g.
                          ``/mnt/user-data/outputs/report.pdf`` or
        ...
        acp_prefix = ACP_VIRTUAL_PATH_PREFIX.lstrip("/")
        prefix = VIRTUAL_PATH_PREFIX.lstrip("/")

        if stripped == acp_prefix or stripped.startswith(acp_prefix + "/"):
            relative = stripped[len(acp_prefix):].lstrip("/")
            base = self.acp_workspace_dir(thread_id).resolve()
        elif stripped == prefix or stripped.startswith(prefix + "/"):
            relative = stripped[len(prefix):].lstrip("/")
            base = self.sandbox_user_data_dir(thread_id).resolve()
        else:
            raise ValueError(f"Path must start with /{prefix} or /{acp_prefix}")
