...
class TruncateArgsConfig(BaseModel):
    """Settings for truncating large tool-call arguments in older messages.

    Lightweight pre-summarization optimization: clips large write_file/edit_file
    args in messages older than the keep window.
    """

    trigger: ContextSize | None = Field(
        default=None,
        description="Threshold that activates argument truncation. Uses the same ContextSize format as summarization trigger.",
    )
    keep: ContextSize | None = Field(
        default=None,
        description="How many recent messages to leave untouched during truncation.",
    )
    max_length: int | None = Field(
        default=2000,
        description="Character limit per argument value before it is clipped.",
    )
    truncation_text: str | None = Field(
        default="...(argument truncated)",
        description="Replacement suffix appended after the first 20 characters of a truncated argument.",
    )
