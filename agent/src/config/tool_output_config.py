...
Ported from deer-flow (deerflow.config.tool_output_config). Adapted to the
nous-agent ``src.*`` package layout.
"""
...
class ToolOutputConfig(BaseModel):
    ...
    externalize_min_chars: int = Field(
        ...
        ge=0,
        description=(
            "Character threshold to trigger disk externalization. Outputs below "
            "this pass through unchanged. Set to 0 to disable externalization "
            "(fallback truncation still applies when output exceeds fallback_max_chars)."
        ),
    )
    ...
    tool_overrides: dict[str, int] = Field(
        ...
        description=(
            "Per-tool externalize_min_chars overrides. Keys are tool names, values "
            "are char thresholds. Use 0 to disable externalization for a specific tool."
        ),
    )


def load_tool_output_config_from_dict(data: dict | None) -> ToolOutputConfig:
    """Build a ToolOutputConfig from a raw config mapping (or defaults when absent)."""
    if not data:
        return ToolOutputConfig()
    return ToolOutputConfig(**data)
