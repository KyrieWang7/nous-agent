...
ACP_VIRTUAL_PATH_PREFIX = "/mnt/acp-workspace"
...
def replace_virtual_path(path: str, thread_data: ThreadDataState | None) -> str:
    """Replace virtual /mnt/user-data or /mnt/acp-workspace paths with actual thread data paths.

    Mapping:
        /mnt/user-data/workspace/* -> thread_data['workspace_path']/*
        /mnt/user-data/uploads/* -> thread_data['uploads_path']/*
        /mnt/user-data/outputs/* -> thread_data['outputs_path']/*
        /mnt/acp-workspace/* -> thread_data['acp_workspace_path']/*
    ...
    """
    if path.startswith(ACP_VIRTUAL_PATH_PREFIX):
        if thread_data is None:
            return path
        acp_base = thread_data.get("acp_workspace_path")
        if acp_base is None:
            return path
        relative = path[len(ACP_VIRTUAL_PATH_PREFIX):].lstrip("/")
        if relative:
            return f"{acp_base}/{relative}"
        return acp_base
...
def replace_virtual_paths_in_command(command: str, thread_data: ThreadDataState | None) -> str:
    """Replace all virtual /mnt/user-data and /mnt/acp-workspace paths in a command string.
    ...
    has_user_data = VIRTUAL_PATH_PREFIX in command
    has_acp = ACP_VIRTUAL_PATH_PREFIX in command

    if not has_user_data and not has_acp:
        return command

    prefixes = []
    if has_acp:
        prefixes.append(re.escape(ACP_VIRTUAL_PATH_PREFIX))
    if has_user_data:
        prefixes.append(re.escape(VIRTUAL_PATH_PREFIX))

    pattern = re.compile(rf"({'|'.join(prefixes)})(/[^\s\"';&|<>()]*)?")
...
def _needs_path_replacement(runtime: ToolRuntime[ContextT, ThreadState] | None) -> bool:
    """Check if virtual path replacement is needed for sandbox tools.

    Path replacement is only needed for LocalSandboxProvider where there
    is no sandbox container — commands execute directly on the host
    filesystem, so /mnt/user-data must be mapped to real paths.

    For AIO sandbox (both auto_start and static modes), the sandbox
    container has /mnt/user-data available, so no replacement is needed.
    Files are synced to MinIO by ``present_files`` for gateway access.
    """
    return is_local_sandbox(runtime)
...
def ensure_thread_directories_exist(runtime: ToolRuntime[ContextT, ThreadState] | None) -> None:
    ...
    Only needed for local sandbox where files live on the host filesystem.
    For AIO sandbox, files are in the container and synced via MinIO.
...
def bash_tool(runtime: ToolRuntime[ContextT, ThreadState], description: str, command: str) -> str:
    ...
    try:
        ...
        if _needs_path_replacement(runtime):
            thread_data = get_thread_data(runtime)
...
def write_file_tool(
    ...
    """
    if path.startswith(ACP_VIRTUAL_PATH_PREFIX):
        return "Error: /mnt/acp-workspace is read-only. ACP agent outputs cannot be modified."
    try:
        sandbox = ensure_sandbox_initialized(runtime)
        ensure_thread_directories_exist(runtime)
        if _needs_path_replacement(runtime):
