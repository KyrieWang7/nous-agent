"""Summarization middleware for Nous Agent.

Ported from DeerFlow's ``DeerFlowSummarizationMiddleware``:
- LLM-based summarization via langchain's ``SummarizationMiddleware``
- pre-compression hook dispatch (before_summarization)
- dynamic-context reminder rescue across compaction
- nostream-tagged summary model to avoid phantom streaming to the frontend
- single factory for both automatic (lead agent) and manual compaction paths
"""
...
class NousSummarizationMiddleware(SummarizationMiddleware):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # the factory for RunJournal attribution): RunnableBinding.with_config does a
    ...
    def _build_summary_input_text(self, formatted_messages: str, previous_summary: str | None = None) -> str | None:
        ...
        # blocks. An unescaped value like "</new_messages>..." would close the block
        # and forge an authority section for the extraction LLM (block-breakout
        # defense). Escape after trimming so a trailing "..." cannot split an entity;
        # quote=False because content lands in element-text position.
        parts: list[str] = []
    ...
    def _preserve_dynamic_context_reminders(
        ...
        The ID-swap triplet contains ``SystemMessage(id=X)`` and
        ``HumanMessage(id=X__memory)`` (both tagged with ``dynamic_context_reminder=True``),
        plus ``HumanMessage(id=X__user)`` carrying the original user content (untagged).
        Without peer rescue, ``__user`` would stay in ``to_summarize`` and be compressed
        into prose — orphaning the tagged messages and losing the user question from the
        model's direct context.
        """
        reminders = [msg for msg in messages_to_summarize if is_dynamic_context_reminder(msg)]
        if not reminders:
            return messages_to_summarize, preserved_messages

        # Collect base IDs (stable_id prefix) from tagged reminders.
        # removesuffix is suffix-only — only known ID-swap suffixes (__memory, __user)
        # are stripped.
        ...
        # Single-pass partition: rescue tagged reminders and untagged ID-swap peers
        # (id starts with a known base + "__"), preserving original order.
...
def create_summarization_middleware(
    *,
    ...
    before_summarization: list[BeforeSummarizationHook] | None = None,
) -> NousSummarizationMiddleware | None:
    """Create the configured summarization middleware.

    Both the lead-agent automatic path and any future manual compaction path
    use this factory so model resolution, prompt config, and retention
    defaults cannot drift.

    Returns ``None`` when summarization is disabled in config.
    """
    ...
    return NousSummarizationMiddleware(
        **kwargs,
