...
Ported from deer-flow (deerflow.agents.middlewares.skill_activation_middleware),
adapted to nous-agent: resolves skills via ``src.skills.load_skills`` and the
``skills`` config section instead of the SkillStorage abstraction.

When the user types ``/skill-name <task>``, this middleware injects the full
SKILL.md content as a hidden HumanMessage right before the user's message, so the
model is guaranteed to follow that skill for the turn. The injected message is
marked ``hide_from_ui`` and keyed off the target message id, so it is idempotent
across turns and does not pollute persisted history shown to the user.
"""
...
from src.skills.types import SKILL_MD_FILE
from src.utils.messages import get_original_user_content_text
...
class SkillActivationMiddleware(AgentMiddleware):
    ...
    def _resolve_activation(self, text: str) -> _ActivationResolution | None:
        ...
        try:
            skill_content = self._read_skill_content(resolved.skill.skill_file, self._skills_root())
