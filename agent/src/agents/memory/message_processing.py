...
Ported from DeerFlow — adds:
- Upload block filtering (strips ephemeral file paths)
- Correction detection (中英双语 regex)
- Reinforcement detection (positive signals)
"""
...
def filter_messages_for_memory(messages: list[Any]) -> list[Any]:
    ...
    - The <uploaded_files> block from human messages (ephemeral file paths)
    - Upload-only turns where no real user text remains after stripping
    """
    filtered = []
    skip_next_ai = False
    for msg in messages:
        ...
        if msg_type == "human":
            ...
            if "<uploaded_files>" in content_str:
                ...
                if not stripped:
                    # Nothing left — entire turn was upload bookkeeping
                    skip_next_ai = True
                    continue
                clean_msg = copy(msg)
