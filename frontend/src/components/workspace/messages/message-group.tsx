...
export function MessageGroup({
  ...
  return (
    <ChainOfThought
      className={cn("w-full gap-2 rounded-2xl border border-border/60 bg-card p-0.5 shadow-sm", className)}
      open={true}
    >
...
function ToolCall({
  ...
  if (name === "web_search") {
    ...
    return (
      <ChainOfThoughtStep key={id} label={label} icon={SearchIcon} isLoading={showTimer}>
    ...
    return (
      <ChainOfThoughtStep key={id} label={description} icon={FolderOpenIcon} isLoading={showTimer}>
        {path && (
    ...
    return (
      <ChainOfThoughtStep key={id} label={description} icon={BookOpenTextIcon} isLoading={showTimer}>
        {path && (
          <ChainOfThoughtSearchResult className="cursor-pointer">
            {path}
          </ChainOfThoughtSearchResult>
    ...
    const displayPath = path ?? "writing...";
    return (
      <ChainOfThoughtStep
        key={id}
        className="cursor-pointer"
        ...
        onClick={() => {
          select(
            new URL(
              `write-file:${displayPath}?message_id=${messageId}&tool_call_id=${id}`,
            ).toString(),
          );
          setOpen(true);
        }}
